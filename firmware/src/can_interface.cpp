...
bool CANInterface::initialize() {
    ...
    // Configure pins before begin for reliable pin muxing
    can1.setTX((FLEXCAN_PINS)PIN_CAN_TX);
    can1.setRX((FLEXCAN_PINS)PIN_CAN_RX);
    can1.begin();
    can1.setBaudRate(CAN_BAUDRATE);
    can1.setMaxMB(16);  // Set maximum mailboxes
    can1.enableFIFO();
    // Disable loopback for real bus operation; enable only for self-test
    // can1.enableLoopBack();
    // We are polling with read(), so no FIFO interrupt is needed
